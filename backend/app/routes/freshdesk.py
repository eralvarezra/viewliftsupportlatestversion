...
async def get_agent_report(
    ...
    start_before = (d_start - timedelta(days=1)).isoformat()
    end_after    = (d_end   + timedelta(days=1)).isoformat()

    fd_id_set = {a["fd_id"] for a in B2C_AGENTS}
    stats = {
        a["fd_id"]: {
            "name": a["name"],
            "fd_id": a["fd_id"],
    ...
    def fetch_resolved(fd_id):
        _, total = fd_search(
            f"(status:4 OR status:5) AND agent_id:{fd_id} AND resolved_at:>'{start_before}' AND resolved_at:<'{end_after}'"
        )
        return fd_id, total

    def fetch_assigned(fd_id):
        _, total = fd_search(
            f"agent_id:{fd_id} AND updated_at:>'{start_before}' AND updated_at:<'{end_after}'"
        )
        return fd_id, total

    def fetch_ticket_ids(fd_id):
        ids = []

        def paginate(base_query, first_results, total):
            ids.extend(t["id"] for t in first_results)
            if total > len(first_results):
                for page in range(2, 11):
                    r2, _ = fd_search(base_query, page)
                    ids.extend(t["id"] for t in r2)
                    if len(r2) < 30:
                        break

        def scan(win_start, win_end):
            base = f"agent_id:{fd_id} AND updated_at:>'{win_start}' AND updated_at:<'{win_end}'"
            results, total = fd_search(base)
            d1 = date.fromisoformat(win_start)
            d2 = date.fromisoformat(win_end)
            span = (d2 - d1).days
            if total > 300 and span > 2:
                mid = d1 + timedelta(days=span // 2)
                scan(win_start, (mid + timedelta(days=1)).isoformat())
                scan(mid.isoformat(), win_end)
            else:
                paginate(base, results, total)

        scan(start_before, end_after)
        return ids
    ...
    with ThreadPoolExecutor(max_workers=30) as pool:
        res_futs  = [pool.submit(fetch_resolved, a["fd_id"]) for a in B2C_AGENTS]
        asgn_futs = [pool.submit(fetch_assigned, a["fd_id"]) for a in B2C_AGENTS]
        id_futs   = [pool.submit(fetch_ticket_ids, a["fd_id"]) for a in B2C_AGENTS]

        for f in as_completed(res_futs):
            fd_id, count = f.result()
            stats[fd_id]["resolved"] = count

        for f in as_completed(asgn_futs):
            fd_id, count = f.result()
            stats[fd_id]["assigned"] = count

        for f in as_completed(id_futs):
            all_ticket_ids.update(f.result())

    ticket_list = list(all_ticket_ids)
    with ThreadPoolExecutor(max_workers=12) as pool:
        conv_futs = [pool.submit(fetch_conversations, tid) for tid in ticket_list]
        for f in as_completed(conv_futs):
            convs = f.result()
            for conv in convs:
                uid = conv.get("user_id")
                if uid not in fd_id_set:
                    continue
                raw_ts = conv.get("created_at") or ""
                if not raw_ts:
                    continue
                try:
                    ...
                    local_date = dt_utc.astimezone(_CR_TZ).date().isoformat()
                except Exception:
                ...
                if conv.get("private"):
                    stats[uid]["notes"] += 1
                elif not conv.get("incoming"):
                    stats[uid]["responses"] += 1

    agents_list = sorted(stats.values(), key=lambda x: x["name"])
    for a in agents_list:
        a["total"] = a["responses"] + a["notes"]

    totals = {
        "responses": sum(a["responses"] for a in agents_list),
        "notes":     sum(a["notes"]     for a in agents_list),
        "total":     sum(a["total"]     for a in agents_list),
        "resolved":  sum(a["resolved"]  for a in agents_list),
        "assigned":  sum(a["assigned"]  for a in agents_list),
    }

    return {"agents": agents_list, "totals": totals, "period": {"start": start, "end": end}}
